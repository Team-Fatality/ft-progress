const ui = document.getElementById('ftprogress');
let running = false;
...
window.onData = function (data) {
    if ( data.MiniGame ) {
        ...
        if (data.display && !running) {
            if ( data.Type == 'linear' ) {
                customDial = new Lineaftprogress({
                    w: data.Width,
                    h: data.Height,
                    x: data.x,
                    y: data.y,
...
function PostData(type, obj) {
    ...
    }).then(resp => resp.json()).then(resp => resp).catch(error => console.log('ftprogress FETCH ERROR! ' + error.message));  
}